...
{
  std::string dms = "CCX_JOBNAME_GETJOBNAME=jobnamec";
  putenv(&dms[0]);
  ...
  // open output file
  // NOTE: for some reason the second arg is not used
  //       and first arg used as the name of output
  ...
  // TODO: reduce stdout to rank 0
  FORTRAN(allocation,
          (&nload_, &nforc_, &nboun_, &nk_, &ne_, &nmpc_, &nset_, &nalset_,
           ...
           &npt_, &nslavs, &nkon_, &mcs, &mortar, &ifacecount, &nintpoint,
           infree, &nheading_, &nobject_, iuel, &iprestr, &nstam, &ndamp,
           &nef));
  ...
  // performing post initialization actions.
  // For now we assume only one step is involved so
  // post initialization will be happening during
...
void clcx_interface::postInit(){
  ...
  if (istep == 0)
    mortar = -1;
  FORTRAN(calinput,
          (co, &nk, kon, ipkon, lakon, &nkon, &ne, nodeboun, ndirboun, xboun,
           &nboun, ipompc, nodempc, coefmpc, &nmpc, &nmpc_, nodeforc, ndirforc,
           xforc, &nforc, &nforc_, nelemload, sideload, xload, &nload, &nload_,
           &nprint, prlab, prset, &mpcfree, &nboun_, mei, set, istartset,
           iendset, ialset, &nset, &nalset, elcon, nelcon, rhcon, nrhcon, alcon,
           nalcon, alzero, t0, t1, matname, ielmat, orname, orab, ielorien,
           amname, amta, namta, &nam, &nmethod, iamforc, iamload, iamt1,
           ithermal, iperturb, &istat, &istep, &nmat, &ntmat_, &norien, prestr,
           &iprestr, &isolver, fei, veold, timepar, xmodal, filab, jout,
           &nlabel, &idrct, jmax, &iexpl, &alpha, iamboun, plicon, nplicon,
           plkcon, nplkcon, &iplas, &npmat_, mi, &nk_, trab, inotr, &ntrans,
           ikboun, ilboun, ikmpc, ilmpc, ics, dcs, &ncs_, &namtot_, cs,
           &nstate_, &ncmat_, &iumat, &mcs, labmpc, iponor, xnor, knor, thickn,
           thicke, ikforc, ilforc, offset, iponoel, inoel, rig, infree, nshcon,
           shcon, cocon, ncocon, physcon, &nflow, ctrl, &maxlenmpc, &ne1d,
           &ne2d, &nener, vold, nodebounold, ndirbounold, xbounold, xforcold,
           xloadold, t1old, eme, sti, ener, xstate, jobnamec, irstrt, &ttime,
           qaold, output, typeboun, inpc, ipoinp, inp, tieset, tietol, &ntie,
           fmpc, cbody, ibody, xbody, &nbody, &nbody_, xbodyold, &nam_, ielprop,
           &nprop, &nprop_, prop, &itpamp, &iviewfile, ipoinpc, &nslavs, t0g,
           t1g, &network, &cyclicsymmetry, idefforc, idefload, idefbody,
           &mortar, &ifacecount, islavsurf, pslavsurf, clearini, heading,
           &iaxial, &nobject, objectset, &nprint_, iuel, &nuel_, nodempcref,
           coefmpcref, ikmpcref, &memmpcref_, &mpcfreeref, &maxlenmpcref,
           &memmpc_, &isens, &namtot, &nstam, dacon, vel, &nef, velo, veloo));
  ...
  if (istat < 0) {
    // break;
    // used to break master loop here
    // instead we just inform user analysis is finished
    // and return.
    message("Calculix Analysis is finished");
    return;
  }
  ...
  if (istep == 1) {
    ...
    tiedcontact(&ntie, tieset, &nset, set, istartset, iendset, ialset, lakon,
                ipkon, kon, tietol, &nmpc, &mpcfree, &memmpc_, &ipompc, &labmpc,
                &ikmpc, &ilmpc, &fmpc, &nodempc, &coefmpc, ithermal, co, vold,
                &nef, &nmpc_, mi, &nk, &istep, ikboun, &nboun, kind1, kind2);
    ...
    FORTRAN(spcmatch, (xboun, nodeboun, ndirboun, &nboun, xbounold, nodebounold,
                       ndirbounold, &nbounold, ikboun, ilboun, vold, reorder,
                       nreorder, mi));
    RENEW(xbounold, double, nboun);
    RENEW(nodebounold, ITG, nboun);
    RENEW(ndirbounold, ITG, nboun);
  ...
  if (network > 0) {
    ...
    FORTRAN(genadvecelem, (inodesd, ipkon, &ne, lakon, kon, &nload, sideload,
                           nelemload, &nkon, &network));
  ...
  if (ithermal[1] >= 2) {
    ...
    FORTRAN(ufaceload, (co, ipkon, kon, lakon, &nboun, nodeboun, nelemload,
                        sideloadtemp, &nload, &ne, &nk));
    SFREE(sideloadtemp);
  }
  ...
  if (((istep == 1) || (ntrans > 0) || (mpcend < 0) || (nk != nkold) ||
    ...
    cascade(ipompc, &coefmpc, &nodempc, &nmpc, &mpcfree, nodeboun, ndirboun,
            &nboun, ikmpc, ilmpc, ikboun, ilboun, &mpcend, labmpc, &nk,
            &memmpc_, &icascade, &maxlenmpc, &callfrommain, iperturb, ithermal);
  }
  ...
  if ((mcs == 0) || (cs[1] < 0)) {
    ...
    if ((icascade == 0) && ((nmethod < 8) || (nmethod > 10))) {
      ...
      mastruct(&nk, kon, ipkon, lakon, &ne, nodeboun, ndirboun, &nboun, ipompc,
               ...
               ikmpc, ilmpc, ipointer, nzs, &nmethodl, ithermal, ikboun, ilboun,
               iperturb, mi, &mortar, typeboun, labmpc, &iit, &icascade,
               &network);
    } else {
      neq[0] = 1;
      neq[1] = 1;
    ...
    mastructcs(&nk, kon, ipkon, lakon, &ne, nodeboun, ndirboun, &nboun, ipompc,
               nodempc, &nmpc, nactdof, icol, jq, &mast1, &irow, &isolver, neq,
               ikmpc, ilmpc, ipointer, nzs, &nmethod, ics, cs, labmpc, &mcs, mi,
               &mortar);
  }
...
void clcx_interface::run() {
  message("Running ...");
  message("Selected Analysis Method #" + std::to_string(nmethod));
  ...
  if ((nmethod <= 1) || (nmethod == 11) ||
    ...
    if (iperturb[0] < 2) {
      ...
      linstatic(co, &nk, &kon, &ipkon, &lakon, &ne, nodeboun, ndirboun, xboun,
                ...
                alzero, &ielmat, &ielorien, &norien, orab, &ntmat_, t0, t1,
                t1old, ithermal, prestr, &iprestr, vold, iperturb, sti, nzs,
                &kode, filab, eme, &iexpl, plicon, nplicon, plkcon, nplkcon,
                &xstate, &npmat_, matname, &isolver, mi, &ncmat_, &nstate_, cs,
                &mcs, &nkon, &ener, xbounold, xforcold, xloadold, amname, amta,
                namta, &nam, iamforc, iamload, iamt1, iamboun, &ttime, output,
                set, &nset, istartset, iendset, ialset, &nprint, prlab, prset,
                &nener, trab, inotr, &ntrans, fmpc, cbody, ibody, xbody, &nbody,
                xbodyold, timepar, thicke, jobnamec, tieset, &ntie, &istep,
                &nmat, ielprop, prop, typeboun, &mortar, mpcinfo, tietol, ics,
                &icontact, orname);
      ...
      nonlingeo(
          ...
          prestr, &iprestr, &vold, iperturb, sti, nzs, &kode, filab, &idrct,
          jmax, jout, timepar, eme, xbounold, xforcold, xloadold, veold, accold,
          amname, amta, namta, &nam, iamforc, &iamload, iamt1, &alpha, &iexpl,
          iamboun, plicon, nplicon, plkcon, nplkcon, &xstate, &npmat_, &istep,
          &ttime, matname, qaold, mi, &isolver, &ncmat_, &nstate_, &iumat, cs,
          &mcs, &nkon, &ener, mpcinfo, output, shcon, nshcon, cocon, ncocon,
          physcon, &nflow, ctrl, set, &nset, istartset, iendset, ialset,
          ...
          &clearini, &nmat, xmodal, &iaxial, &inext, &nprop, &network, orname,
          vel, &nef, velo, veloo);
    ...
    if ((mcs == 0) || (cs[1] < 0)) {
      ...
      arpack(co, &nk, &kon, &ipkon, &lakon, &ne, nodeboun, ndirboun, xboun,
             &nboun, ipompc, nodempc, coefmpc, labmpc, &nmpc, nodeforc,
             ndirforc, xforc, &nforc, nelemload, sideload, xload, &nload,
             nactdof, icol, jq, &irow, neq, &nzl, &nmethod, ikmpc, ilmpc,
             ikboun, ilboun, elcon, nelcon, rhcon, nrhcon, shcon, nshcon, cocon,
             ncocon, alcon, nalcon, alzero, &ielmat, &ielorien, &norien, orab,
             &ntmat_, t0, t1, t1old, ithermal, prestr, &iprestr, vold, iperturb,
             sti, nzs, &kode, mei, fei, filab, &iexpl, plicon, nplicon, plkcon,
             nplkcon, &xstate, &npmat_, matname, mi, &ncmat_, &nstate_, &ener,
             jobnamec, output, set, &nset, istartset, iendset, ialset, &nprint,
             prlab, prset, &nener, &isolver, trab, inotr, &ntrans, &ttime, fmpc,
             cbody, ibody, xbody, &nbody, thicke, &nslavs, tietol, &nkon,
             mpcinfo, &ntie, &istep, &mcs, ics, tieset, cs, &nintpoint, &mortar,
             &ifacecount, &islavsurf, &pslavsurf, &clearini, &nmat, typeboun,
             ielprop, prop, orname);
      ...
      arpackcs(co, &nk, &kon, &ipkon, &lakon, &ne, nodeboun, ndirboun, xboun,
               &nboun, ipompc, nodempc, coefmpc, labmpc, &nmpc, nodeforc,
               ndirforc, xforc, &nforc, nelemload, sideload, xload, &nload,
               nactdof, icol, jq, &irow, neq, &nzl, &nmethod, ikmpc, ilmpc,
               ikboun, ilboun, elcon, nelcon, rhcon, nrhcon, alcon, nalcon,
               alzero, &ielmat, &ielorien, &norien, orab, &ntmat_, t0, t1,
               t1old, ithermal, prestr, &iprestr, vold, iperturb, sti, nzs,
               &kode, mei, fei, filab, &iexpl, plicon, nplicon, plkcon, nplkcon,
               &xstate, &npmat_, matname, mi, ics, cs, &mpcend, &ncmat_,
               &nstate_, &mcs, &nkon, jobnamec, output, set, &nset, istartset,
               iendset, ialset, &nprint, prlab, prset, &nener, &isolver, trab,
               inotr, &ntrans, &ttime, fmpc, cbody, ibody, xbody, &nbody,
               &nevtot, thicke, &nslavs, tietol, mpcinfo, &ntie, &istep, tieset,
               &nintpoint, &mortar, &ifacecount, &islavsurf, &pslavsurf,
               &clearini, &nmat, typeboun, ielprop, prop, orname);
    ...
    arpackbu(co, &nk, kon, ipkon, lakon, &ne, nodeboun, ndirboun, xboun, &nboun,
             ...
             &nforc, nelemload, sideload, xload, &nload, nactdof, icol, jq,
             irow, neq, &nzl, &nmethod, ikmpc, ilmpc, ikboun, ilboun, elcon,
             nelcon, rhcon, nrhcon, alcon, nalcon, alzero, ielmat, ielorien,
             &norien, orab, &ntmat_, t0, t1, t1old, ithermal, prestr, &iprestr,
             vold, iperturb, sti, nzs, &kode, mei, fei, filab, eme, &iexpl,
             plicon, nplicon, plkcon, nplkcon, xstate, &npmat_, matname, mi,
             &ncmat_, &nstate_, ener, output, set, &nset, istartset, iendset,
             ialset, &nprint, prlab, prset, &nener, &isolver, trab, inotr,
    ...
    dyna(&co, &nk, &kon, &ipkon, &lakon, &ne, &nodeboun, &ndirboun, &xboun,
         &nboun, &ipompc, &nodempc, &coefmpc, &labmpc, &nmpc, nodeforc,
         ndirforc, xforc, &nforc, nelemload, sideload, xload, &nload, &nactdof,
         neq, &nzl, icol, irow, &nmethod, &ikmpc, &ilmpc, &ikboun, &ilboun,
         elcon, nelcon, rhcon, nrhcon, cocon, ncocon, alcon, nalcon, alzero,
         &ielmat, &ielorien, &norien, orab, &ntmat_, &t0, &t1, ithermal, prestr,
         &iprestr, &vold, iperturb, &sti, nzs, timepar, xmodal, &veold, amname,
         amta, namta, &nam, iamforc, iamload, &iamt1, jout, &kode, filab, &eme,
         xforcold, xloadold, &t1old, &iamboun, &xbounold, &iexpl, plicon,
         nplicon, plkcon, nplkcon, &xstate, &npmat_, matname, mi, &ncmat_,
         &nstate_, &ener, jobnamec, &ttime, set, &nset, istartset, iendset,
         &ialset, &nprint, prlab, prset, &nener, trab, &inotr, &ntrans, &fmpc,
         cbody, ibody, xbody, &nbody, xbodyold, &istep, &isolver, jq, output,
         &mcs, &nkon, &mpcend, ics, cs, &ntie, tieset, &idrct, jmax, ctrl,
         &itpamp, tietol, &nalset, ikforc, ilforc, thicke, &nslavs, &nmat,
    ...
    steadystate(
        &co, &nk, &kon, &ipkon, &lakon, &ne, &nodeboun, &ndirboun, &xboun,
        &nboun, &ipompc, &nodempc, &coefmpc, &labmpc, &nmpc, nodeforc, ndirforc,
        xforc, &nforc, nelemload, sideload, xload, &nload, &nactdof, neq, &nzl,
        icol, irow, &nmethod, &ikmpc, &ilmpc, &ikboun, &ilboun, elcon, nelcon,
        rhcon, nrhcon, cocon, ncocon, alcon, nalcon, alzero, &ielmat, &ielorien,
        &norien, orab, &ntmat_, &t0, &t1, ithermal, prestr, &iprestr, &vold,
        iperturb, sti, nzs, timepar, xmodal, &veold, amname, amta, namta, &nam,
        iamforc, iamload, &iamt1, jout, &kode, filab, &eme, xforcold, xloadold,
        &t1old, &iamboun, &xbounold, &iexpl, plicon, nplicon, plkcon, nplkcon,
        xstate, &npmat_, matname, mi, &ncmat_, &nstate_, &ener, jobnamec,
        &ttime, set, &nset, istartset, iendset, ialset, &nprint, prlab, prset,
        &nener, trab, &inotr, &ntrans, &fmpc, cbody, ibody, xbody, &nbody,
    ...
    complexfreq(
        ...
        &ntie, tieset, &idrct, jmax, ctrl, &itpamp, tietol, &nalset, ikforc,
        ilforc, thicke, jobnamef, mei, &nmat, ielprop, prop, orname, typeboun);
    ...
    electromagnetics(
        ...
        nelcon, rhcon, nrhcon, alcon, nalcon, alzero, &ielmat, &ielorien,
        &norien, orab, &ntmat_, t0, t1, t1old, ithermal, prestr, &iprestr,
        &vold, iperturb, sti, nzs, &kode, filab, &idrct, jmax, jout, timepar,
        eme, xbounold, xforcold, xloadold, veold, accold, amname, amta, namta,
        &nam, iamforc, &iamload, iamt1, &alpha, &iexpl, iamboun, plicon,
        nplicon, plkcon, nplkcon, &xstate, &npmat_, &istep, &ttime, matname,
        qaold, mi, &isolver, &ncmat_, &nstate_, &iumat, cs, &mcs, &nkon, &ener,
        mpcinfo, output, shcon, nshcon, cocon, ncocon, physcon, &nflow, ctrl,
        &set, &nset, &istartset, &iendset, &ialset, &nprint, prlab, prset,
        &nener, ikforc, ilforc, trab, inotr, &ntrans, &fmpc, cbody, ibody,
        xbody, &nbody, xbodyold, ielprop, prop, &ntie, &tieset, &itpamp,
        &iviewfile, jobnamec, &tietol, &nslavs, thicke, ics, &nalset, &nmpc_,
        &nmat, typeboun, &iaxial, &nload_, &nprop, &network, orname);
    ...
    sensitivity(
        ...
        &ntmat_, t0, t1, t1old, ithermal, prestr, &iprestr, vold, iperturb, sti,
        nzs, &kode, filab, eme, &iexpl, plicon, nplicon, plkcon, nplkcon,
        &xstate, &npmat_, matname, &isolver, mi, &ncmat_, &nstate_, cs, &mcs,
        &nkon, &ener, xbounold, xforcold, xloadold, amname, amta, namta, &nam,
        iamforc, iamload, iamt1, iamboun, &ttime, output, set, &nset, istartset,
        iendset, ialset, &nprint, prlab, prset, &nener, trab, inotr, &ntrans,
        fmpc, cbody, ibody, xbody, &nbody, xbodyold, timepar, thicke, jobnamec,
        tieset, &ntie, &istep, &nmat, ielprop, prop, typeboun, &mortar, mpcinfo,
        tietol, ics, &icontact, &nobject, &objectset, &istat, orname,
        nzsprevstep, &nlabel, physcon, jobnamef);
  }
...
int clcx_interface::message(std::string msg) {
  if (myid == 0)
    std::cout << "Calculix : " << msg << std::endl;
  return (1);
}

void clcx_interface::splash() {
  message("************************************************************");
  message("CalculiX Version 2.15, Copyright(C) 1998-2018 Guido Dhondt");
  message("(IMPACT CSC Module)");
  message("CalculiX comes with ABSOLUTELY NO WARRANTY. This is free");
  message("software, and you are welcome to redistribute it under");
  message("certain conditions, see gpl.htm");
  message("************************************************************");
  message("You are using a CSC module made on Wed Oct 23 13:41:18 CDT 2019");
}

void clcx_interface::step_check() {
...
void clcx_interface::step_initialize() {
...
void clcx_interface::step_run(const double step_tt) {
...
bool clcx_interface::step_completed() { return (nlgeo->is_completed()); }

void clcx_interface::step_finalize() {
...
void clcx_interface::step(const double &stp_tt) {
  step_initialize();
...
#define STOREP(a,b) \
    {\
        ...
        ar & boost::serialization::make_array<b>(a,dvar[#a]);\
    }
...
void clcx_interface::serialize(Archive &ar, const unsigned int version) 
{
  ...
  ar & jobnamef;
  ar & output;
  STOREP(typeboun, char);
  STOREP(inpc, char);
  STOREP(tieset, char);
  STOREP(cbody, char);
  ar & fneig;
  STOREP(sideloadtemp, char);
  ar & kind1;
  ar & kind2;
  STOREP(heading, char);
  STOREP(objectset, char);
  ...
  STOREP(ipointer, ITG);
  STOREP(idefload, ITG);
  STOREP(istartset, ITG);
  STOREP(iendset, ITG);
  STOREP(ialset, ITG);
  STOREP(ielmat, ITG);
  STOREP(ielorien, ITG);
  STOREP(nrhcon, ITG);
  STOREP(nodebounold, ITG);
  STOREP(ndirbounold, ITG);
  STOREP(nelcon, ITG);
  STOREP(nalcon, ITG);
  STOREP(iamforc, ITG);
  STOREP(iamload, ITG);
  ...
  STOREP(ibody, ITG);
  STOREP(ielprop, ITG);
  STOREP(islavsurf, ITG);
  STOREP(ipoinpc, ITG);
  STOREP(iuel, ITG);

  STOREP(ipoinp, ITG);
  STOREP(inp, ITG);
  STOREP(nodempcref, ITG);
  STOREP(ikmpcref, ITG);

  STOREP(meminset, ITG);
  STOREP(rmeminset, ITG);

  STOREP(co, double);
  STOREP(xboun, double);
  STOREP(coefmpc, double);
  STOREP(xforc, double);
  STOREP(clearini, double);
  STOREP(xload, double);
  STOREP(xbounold, double);
  STOREP(xforcold, double);
  STOREP(vold, double);
  STOREP(sti, double);
  STOREP(xloadold, double);
  STOREP(xnor, double);
  STOREP(reorder, double);
  STOREP(dcs, double);
  STOREP(thickn, double);
  STOREP(thicke, double);
  STOREP(offset, double);
  STOREP(elcon, double);
  STOREP(rhcon, double);
  STOREP(alcon, double);
  STOREP(alzero, double);
  STOREP(t0, double);
  STOREP(t1, double);
  STOREP(prestr, double);
  STOREP(orab, double);
  STOREP(amta, double);
  STOREP(veold, double);
  STOREP(accold, double);
  STOREP(t1old, double);
  STOREP(eme, double);
  STOREP(plicon, double);
  STOREP(pslavsurf, double);
  STOREP(plkcon, double);
  STOREP(xstate, double);
  STOREP(trab, double);
  STOREP(ener, double);
  STOREP(shcon, double);
  STOREP(cocon, double);
  STOREP(cs, double);
  STOREP(tietol, double);
  STOREP(fmpc, double);
  STOREP(prop, double);
  STOREP(t0g, double);
  STOREP(t1g, double);
  STOREP(xbody, double);
  STOREP(xbodyold, double);
  STOREP(coefmpcref, double);
  STOREP(dacon, double);
  STOREP(vel, double);
  STOREP(velo, double);
  STOREP(veloo, double);

  STOREP(xmodal, double);
