...
int testHomogenization(const std::string &testFN, const std::string &refFN) {
  ...
  _ci->postInit();
  _ci->run();
  return compareFiles("homogenization.csv", "h_ref.csv");
}

TEST(CSC, Homogenization) {
  EXPECT_EQ(1, testHomogenization(arg_fName1, arg_fName2));
}
